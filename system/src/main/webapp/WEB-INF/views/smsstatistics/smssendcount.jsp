    ...
    *********************************************************************************************************************************************************** -->
	...
	<section id="main-content">
		<section class="wrapper">
			<h3><i class="fa fa-angle-right"></i>统计查询/短信发送统计</h3>
			<div class="row mt">
				<div class="col-lg-12 col-xs-12">
					<h4><i class="fa fa-angle-right"></i>发送统计</h4>
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-lg-4 col-md-4 col-sm-4 mb">
								...
								<select class="form-control smssendcountcp" id="smssendcountcp">
							...
							<div class="col-lg-4 col-md-4 col-sm-4 mb">
								...
								<select class="form-control smssendcountchannel" id="smssendcountchannel">
									<option value="" selected>全部</option>
							...
							<div class="col-lg-4 col-md-4 col-sm-4 mb">
								...
								<select class="form-control smssendcountby" id="smssendcountby">
							...
							<div class="col-lg-5 col-md-5 col-sm-5 btn-group">
								<button type="button" class="btn btn-warning" onclick="checksmssendcount(1);"><i class="glyphicon glyphicon-search"></i>查询</button>
							</div>
						</div>
					</div>
				...
				<div class="table-responsive">
					<table class="table  table-bordered">
						...
						<tfoot class="smssendcountfoot">
						<tr >
							<td colspan="12" align="center">
								<ul class="pagination">
									<ul class="pagination" id="smssendcountpage">
...
<script>
	...
	$("#smssendcountpagecount").append("<li><span>共"+pagesum+"页"+countnum+"条记录</span></li>")
	...
	function changePageNum(pageNum){
		window.location.href = "${basePath}/statistical/smssendcount?pageNum="+pageNum;
	}

	//条件查询数据
	function checksmssendcount(pageNum) {
		var loadingIndex = null;
		var cp = $("#smssendcountcp").val();
		var channel = $("#smssendcountchannel").val();
		var way = $("#smssendcountby").val();
		...
		if(way == ""){
			layer.msg("统计方式不能为空，请选择统计方式", {time:2000, icon:5, shift:2}, function(){});
			return;
		}
		$.ajax({
			type:"GET",
			url:"${basePath}/statistical/checksmssendcount",
			data:{"cp":encodeURIComponent(cp),"channel":encodeURIComponent(channel),"byway":way,"begintime":begintime,"endtime":endtime,"pageNum":pageNum},
			beforeSend:function () {
				loadingIndex =  layer.msg('查询中...', {time:120000, icon:16, shift:10});
			},
			success:function (result) {
				layer.close(loadingIndex);
				var byway = $("#smssendcountby").find("option:selected").text();
				$("#way").text("");
				$("#way").text(byway);
				//清空表格数据
				$(".smssendcountmsg").empty();
				$(".smssendcountfoot").empty();
				//总页码
				...
				for (var i = 0; i < dataStr.length ; i++) {
					var sendnum = dataStr[i].sendNum;
					var filternum = dataStr[i].filterNum;
					var submitnum = dataStr[i].submitNum;
					var successnum = dataStr[i].successNum;
					var failnum = dataStr[i].failNum;
					if (sendnum == null) {
						sendnum = 0;
					}
					if (filternum == null) {
					...
					$(".smssendcountmsg").append("<tr class='smssendcount_tr'>" +
							"<td>"+(i+1)+"</td>" +
							"<td><input type='checkbox' ></td>" +
							"<td>"+dataStr[i].way+"</td>" +
							"<td>"+sendnum+"</td>" +
				...
				//插件分页
				$(".smssendcountfoot").append("<tr><td colspan='12' align='center'><ul class='pagination' id='checksmssendcountpage'></ul><ul id='checkpagecount'></ul></td></tr>");
				$.jqPaginator('#checksmssendcountpage', {
					...
					onPageChange: function (num, type) {
						if (type == "change"){
							checksmssendcount(num);
						}
					}
				});
				$("#checkpagecount").append("<li><span>共"+totalPage+"页"+countNum+"条记录</span></li>")
			}
		});
	}
</script>
