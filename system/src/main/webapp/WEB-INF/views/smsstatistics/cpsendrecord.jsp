			...
			<div class="row mt">
					...
					<div class="panel-body">
						<div class="row">
							<%--<div class="col-lg-4 col-md-4 col-sm-4 mb">--%>
								...
								<%--&lt;%&ndash;<input class="form-control has-success" type="text">&ndash;%&gt;--%>
							...
							<div class="col-lg-5 col-md-5 col-sm-5 mb">
								<button type="button" class="btn btn-warning" onclick="cpchecksendrecord(1);"><i class="glyphicon glyphicon-search"></i>查询</button>
							</div>
						</div>
					</div>
				...
				<div class="table-responsive">
					<table class="table  table-bordered">
						...
						<tfoot class="cprecordfoot">
						<tr >
							<td colspan="13" align="center">
								<ul class="pagination" id="cpquerymsgpage">
									<input type="hidden" value="${result.totalPage}" id="cptotalpage">
									<input type="hidden" value="${result.pageNum}" id="cppagenum">
									<input type="hidden" value="${result.pageSize}" id="cppagesize">
...
<div style="width: 100%;height: 100%;">
	...
	<div class="modal fade" id="cpinfoModal" tabindex="-1" role="dialog" data-backdrop="static">
		<%--设置宽度1000不能适配手机--%>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body" style="height: 700px;">
					<input type="hidden" value="" id="cpcheckinfoId">
					<div class="row mt">
						<div class="col-lg-12 col-xs-12">
							<div class="row">
								<div class="col-lg-6 col-md-6 col-sm-6 mb">
									...
									<%--<input class="form-control has-success" type="text">--%>
									<select class="form-control" id="cpcheckselect">
								...
								<div class="col-lg-6 col-md-6 col-sm-6 mb">
									...
									<input class="form-control has-success" type="text" id="cpcheckmobile">
								</div>
								<div class="col-lg-6 col-md-6 col-sm-6 mb">
							...
							<section id="no-more-tables">
								<div class="table-responsive">
									<table class="table table-bordered table-striped table-condensed cf">
										...
										<tbody class="cpinfomsg">
										</tbody>
										<tfoot class="cpinfofoot">
										</tfoot>
									</table>
								</div>
							</section>
...
<script>
	...
	//使用插件分页
	var pagesum = $("#cptotalpage").val();
	var pageindex = $("#cppagenum").val();
	var pagesize = $("#cppagesize").val();
	var countnum = $("#countnum").val();
	if (pagesum == 0 && countnum == 0){
		pagesum = 1;
	}
	$.jqPaginator('#cpquerymsgpage', {
	...
	$('#cpinfoModal').on('hidden.bs.modal', function (){
		$("#cpcheckmobile").val("");
		$("#cpcheckselect").find("option:selected").attr("selected",false);
		$("#cpcheckselect").find("option").first().attr("selected",true);
	});
	...
	//条件查询数据
	function cpchecksendrecord(pageNum) {
		...
		$.ajax({
			type:"GET",
			url:"${basePath}/statistical/cpcheckrecord",
			...
			success:function (result) {
				...
				for (var i = 0; i < dataStr.length ; i++) {
					...
					$(".cprecordmsg").append("<tr class='cprecord_tr'>" +
							"<td>"+(i+1)+"</td>" +
							"<td><input type='checkbox' ></td>" +
							"<td>"+dataStr[i].cp+"</td>" +
							"<td>"+dataStr[i].id+"</td>" +
				...
				$(".cprecordfoot").append("<tr><td colspan='12' align='center'><ul class='pagination' id='cprecordmsgpage'></ul><ul id='cprecordpagecount'></ul></td></tr>");
				//使用插件分页
				$.jqPaginator('#cprecordmsgpage', {
					...
					onPageChange: function (num, type) {
						if (type == "change"){
							cpchecksendrecord(num);
						}
					}
				});
				$("#cprecordpagecount").append("<li><span>共"+totalPage+"页"+countNum+"条记录</span></li>")
			}
		});
	}
		...
		$.ajax({
			...
			success:function (result) {
				...
				for (var i = 0; i < dataStr.length ; i++) {
					...
					$(".cpinfomsg").append("<tr class='cpinfo_tr'>" +
							"<td>"+dataStr[i].mobile+"</td>" +
							"<td>"+dataStr[i].city+"</td>" +
							"<td>"+dataStr[i].carrier+"</td>" +
							"<td id='cpinfo_"+i+"' title='"+dataStr[i].sendContent+"'>"+dataStr[i].sendContent+"</td>" +
							"<td>"+dataStr[i].cp+"</td>" +
				...
				//动态生成分页页码
				$(".cpinfofoot").append("<tr><td colspan='12' align='center'><ul class='pagination' id='cpinfomsgpage'></ul><ul id='cpinfomsgpagecount'></ul></td></tr>");
				//使用插件分页
				$.jqPaginator('#cpinfomsgpage', {
					totalPages: totalPage,
					visiblePages: pageSize,
					currentPage: pageNum,
					first: '<li class="first"><a href="javascript:void(0);">首页</a></li>',
				...
				$("#cpinfomsgpagecount").append("<li><span>共"+totalPage+"页"+countNum+"条记录</span></li>")

				//处理号码过长展示
				var tr = $(".cpinfo_tr");
				for (var i = 0; i < tr.length ; i++) {

					new $.Zebra_Tooltips($("#cpinfo_"+i+""));
					var td = $("#cpinfo_"+i+"").text();
					if (td.length > 5) {
	...
	function checkinfomsg(pageNum){
		var loadingIndex = null;
		var checkinfoId = $("#cpcheckinfoId").val();
		var checkselect = $("#cpcheckselect").val();
		var checkmobile = $("#cpcheckmobile").val();
		$.ajax({
			type:"POST",
			...
			success:function (result) {
				...
				for (var i = 0; i < dataStr.length ; i++) {
					...
					$(".cpinfomsg").append("<tr class='cp_info_tr'>" +
							"<td>"+dataStr[i].mobile+"</td>" +
							"<td>"+dataStr[i].city+"</td>" +
							"<td>"+dataStr[i].carrier+"</td>" +
							"<td id='cp_info_"+i+"' title='"+dataStr[i].sendContent+"'>"+dataStr[i].sendContent+"</td>" +
							"<td>"+dataStr[i].cp+"</td>" +
							// "<td>"+dataStr[i].channel+"</td>" +
							"<td>"+status+"</td>" +
							"<td>"+isshow+"</td>" +
					...
					//动态生成分页页码
					$(".cpinfofoot").append("<tr><td colspan='12' align='center'><ul class='pagination' id='cp_infomsgpage'></ul><ul id='cpcheckinfopagecount'></ul></td></tr>");
					//使用插件分页
					$.jqPaginator('#cp_infomsgpage', {
						totalPages: totalPage,
						visiblePages: pageSize,
						currentPage: pageNum,
						first: '<li class="first"><a href="javascript:void(0);">首页</a></li>',
				...
				$("#cpcheckinfopagecount").append("<li><span>共"+totalPage+"页"+countNum+"条记录</span></li>")

					//处理号码过长展示
					var tr = $(".cp_info_tr");
					for (var i = 0; i < tr.length ; i++) {

						new $.Zebra_Tooltips($("#cp_info_"+i+""));
						var td = $("#cp_info_"+i+"").text();
						if (td.length > 5) {
							//给td设置title属性,并且设置td的完整值.给title属性.
							// $("#content_"+i+"").attr("title",td);
							//获取td的值,进行截取。赋值给text变量保存.
