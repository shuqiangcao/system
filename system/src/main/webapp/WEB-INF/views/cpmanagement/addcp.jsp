            ...
            <div class="row mt">
                <div class="col-lg-12 col-xs-12">
                    <div class="content-panel">
                        <h4><i class="fa fa-angle-right"></i>新增渠道</h4>
                        <section id="unseen">
                            <div class="panel panel-default">
                                <%--<div class="panel-heading">表单数据<div style="float:right;cursor:pointer;" data-toggle="modal" data-target="#myModal"></div></div>--%>
                                <div class="panel-body">
                                    <form role="form" action="${basePath}/cp/savecp" method="post">
                                        <div class="form-group">
                                            <label for="exampleInputPassword1">渠道名称</label>
                                            <input type="text" class="form-control" id="exampleInputPassword1" placeholder="请输入渠道名称">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="exampleInputPassword5" placeholder="请输入连接数(cmpp方式)">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="exampleInputPassword7" placeholder="请输入渠道权重">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword8">渠道流量</label>
                                            <input type="text" class="form-control" id="exampleInputPassword8" placeholder="请输入渠道流量">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword10">ip白名单</label>
                                            <input type="text" class="form-control" id="exampleInputPassword10" placeholder="请输入ip白名单(cmpp方式)">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword9">回调地址</label>
                                            <input type="text" class="form-control" id="exampleInputPassword9" placeholder="请输入回调地址">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword11">渠道接入方式</label>
                                            <input type="text" class="form-control" id="exampleInputPassword11" placeholder="请输入渠道接入方式">
                                        </div>
                                        <button type="button" class="btn btn-success" onclick="addcp();"><i class="glyphicon glyphicon-plus"></i> 新增</button>
...
<script>
    function addcp() {
        ...
        if(!(accessMode.indexOf("cmppv2") == -1)){
            if(connections == ""){
                layer.msg("渠道连接数不能为空，请输入连接数", {time:2000, icon:5, shift:2}, function(){});
                return;
            }
            if(whiteList == ""){
                layer.msg("ip白名单不能为空，请输入ip", {time:2000, icon:5, shift:2}, function(){});
                return;
            }
        }
        ...
        $.ajax({
            type:"POST",
            url:"${basePath}/cp/savecp",
            data:JSON.stringify({"account":account,"name":name,"passWord":passWord,"connections":connections,"flowRate":flowRate,"accessMode":accessMode,"url":url,"srcId":srcId,"weight":weight,"whiteList":whiteList}),
            contentType:"application/json;charset=UTF-8",
            beforeSend:function () {
                loadingIndex =  layer.msg('处理中...', {icon: 16});
            },
            success:function (result) {
                var cp = result.repeatCp;
                if (result.success){
                    layer.msg("新增渠道成功！", {time:3000, icon:6, shift:2}, function(){});
                    window.setTimeout("window.location.href='${basePath}/cp/cpmessage'",2000);
                }else if(result.repeatCp){
                    layer.msg("渠道已存在！", {time:2000, icon:5, shift:6}, function(){});
                }else{
                    layer.msg("新增渠道失败！", {time:2000, icon:5, shift:6}, function(){});
                }
            }
        });
