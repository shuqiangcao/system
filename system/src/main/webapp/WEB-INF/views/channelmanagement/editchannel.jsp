            ...
            <div class="row mt">
                <div class="col-lg-12 col-xs-12">
                    <div class="content-panel">
                        <h4><i class="fa fa-angle-right"></i>修改通道</h4>
                        <section id="unseen">
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    <form role="form" action="${basePath}/cp/saveeditcp" method="post">
                                        <input type="hidden" value="${smsChannel.id}" id="editchannelid">
                                        <div class="form-group">
                                            <label for="exampleInputPassword1">通道名称</label>
                                            <input type="text" class="form-control" id="exampleInputPassword1" value="${smsChannel.account}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword16">通道别名</label>
                                            <input type="text" class="form-control" id="exampleInputPassword16" value="${smsChannel.alias}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword2">账号</label>
                                            <input type="text" class="form-control" id="exampleInputPassword2" value="${smsChannel.name}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword3">密码</label>
                                            <input type="text" class="form-control" id="exampleInputPassword3" value="${smsChannel.passWord}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword5">连接数</label>
                                            <input type="text" class="form-control" id="exampleInputPassword5" value="${smsChannel.connections}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword6">码号</label>
                                            <input type="text" class="form-control" id="exampleInputPassword6" value="${smsChannel.srcId}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword7">通道权重</label>
                                            <input type="text" class="form-control" id="exampleInputPassword7" value="${smsChannel.weight}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword8">发送速率</label>
                                            <input type="text" class="form-control" id="exampleInputPassword8" value="${smsChannel.flowRate}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword10">ip地址</label>
                                            <input type="text" class="form-control" id="exampleInputPassword10" value="${smsChannel.ip}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword9">端口号</label>
                                            <input type="text" class="form-control" id="exampleInputPassword9" value="${smsChannel.port}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword14">通道地址</label>
                                            <input type="text" class="form-control" id="exampleInputPassword14" value="${smsChannel.url}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword15">秘钥</label>
                                            <input type="text" class="form-control" id="exampleInputPassword15" value="${smsChannel.keyWord}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword11">通道接入方式</label>
                                            <input type="text" class="form-control" id="exampleInputPassword11" value="${smsChannel.accessMode}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword12">价格</label>
                                            <input type="text" class="form-control" id="exampleInputPassword12" value="${smsChannel.price}">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword13">通道屏蔽地区</label>
                                            <input type="text" class="form-control" id="exampleInputPassword13" value="${smsChannel.limitArea}">
                                        </div>
                                        <button type="button" class="btn btn-success" onclick="saveeditchannel();"><i class="glyphicon glyphicon-plus"></i>修改</button>
...
<script>
    function saveeditchannel() {
        ...
        var price = $("#exampleInputPassword12").val();
        var limitarea = $("#exampleInputPassword13").val();
        var id = $("#editchannelid").val();
        ...
        if(flowRate == ""){
            layer.msg("通道发送速率不能为空，请输入发送速率", {time:2000, icon:5, shift:2}, function(){});
            return;
        }
        if(accessMode == ""){
        ...
        if(price == ""){
            layer.msg("通道价格不能为空，请输入通道价格", {time:2000, icon:5, shift:2}, function(){});
            return;
        }
        if(limitarea == ""){
            layer.msg("通道屏蔽地区不能为空，请输入通道屏蔽地区", {time:2000, icon:5, shift:2}, function(){});
            return;
        }
        if (accessMode == "http"){
            if(url == ""){
        ...
        $.ajax({
            type:"POST",
            url:"${basePath}/channel/saveeditchannel",
            data:JSON.stringify({"id":id,"account":account,"alias":alias,"name":name,"passWord":passWord,"connections":connections,"flowRate":flowRate,"accessMode":accessMode,"ip":ip,"srcId":srcId,"weight":weight,"port":port,"price":price,"limitArea":limitarea,"url":url,"keyWord":keyWord}),
            ...
            success:function (result) {
                if (result.success){
                    layer.msg("通道信息修改成功！", {time:3000, icon:6, shift:2}, function(){});
                    window.setTimeout("window.location.href='${basePath}/channel/channelmsg'",2000);
                }else{
                    layer.msg("通道信息修改失败！", {time:2000, icon:5, shift:6}, function(){});
                }
            }
        });
