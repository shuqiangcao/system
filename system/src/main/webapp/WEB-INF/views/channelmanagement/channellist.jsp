    ...
    *********************************************************************************************************************************************************** -->
	...
	<section id="main-content">
		<section class="wrapper">
			<h3><i class="fa fa-angle-right"></i>渠道管理/通道信息</h3>
			<div class="row mt">
				<div>
					<h4><i class="fa fa-angle-right"></i>通道列表</h4>
					<section id="unseen">
						<button type="button" class="btn btn-primary" style="float:left;" onclick="window.location.href='${basePath}/channel/addchannel'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
						<br>
						<br>
						<br>
							...
							<table class="table  table-bordered">
								...
								<c:forEach items="${result.data}" var="item" varStatus="status">
									<tr class="channel_tr">
										...
										<td>${item.alias}</td>
										<td>${item.name}</td>
										<td id="pwd_${status.index}">********<i id="pwd_${status.index}" class='fa fa-eye' onclick="showhide('${item.passWord}',this.id,'true')" style="cursor: pointer"></i></td>
										<td>${item.ip}</td>
										<td>${item.port}</td>
										<td>${item.url}</td>
										<td>${item.keyWord}</td>
										...
										<td>${item.weight}</td>
										<td>${item.flowRate}</td>
										<td>${item.limitArea}</td>
										<td>${item.accessMode}</td>
										<td>
											<button type="button" class="btn btn-primary btn-xs" onclick="editchannel(${item.id});"><i class=" glyphicon glyphicon-pencil">修改</i></button>
										</td>
										<td>
											<button type="button" class="btn btn-danger btn-xs" onclick="deletechannel(${item.id});"><i class=" glyphicon glyphicon-remove">删除</i></button>
										</td>
									</tr>
								</c:forEach>
								</tbody>
								...
								<tr >
									<td colspan="19" align="center">
										<ul class="pagination" id="channelcountpage">
...
<script>
	...
	$("#channelcountpagecount").append("<li><span>共"+pagesum+"页"+countnum+"条记录</span></li>")

	var tdpwd = "";
	function showhide(pwd,id,flag){
		if (flag == "true"){
			tdpwd = pwd;
			flag = "false";
		}
		var channeltr = $(".channel_tr");
		for (var i = 0;i < channeltr.length;i++){
			var pid = "pwd_"+i;
			if (id == pid) {
				var tdvalue = $("#pwd_"+i+"").text();
				if (tdvalue == "********") {
					//$("#pwd_"+i+"").text("");
					$("#pwd_"+i+"").html(""+tdpwd+"<i id='"+pid+"' class='fa fa-eye' onclick=\"showhide('"+tdpwd+"',this.id,"+flag+")\" style='cursor: pointer'></i>");
				}else{
					//$("#pwd_"+i+"").text("");
					$("#pwd_"+i+"").html("********<i id='"+pid+"' class='fa fa-eye' onclick=\"showhide('********',this.id,"+flag+")\" style='cursor: pointer'></i>");
				}
	...
	function editchannel(channelId) {
		window.location.href = "${basePath}/channel/editchannel?channelId="+channelId;
	}
	function deletechannel(channelId) {
		layer.confirm("是否确定删除通道信息?",{icon: 3,title:'提示'},function(cindex){
			$.ajax({
				type:"POST",
				...
				success:function (result) {
					if (result.success){
						layer.msg("删除通道信息成功！", {time:3000, icon:6, shift:2}, function(){});
						window.setTimeout("window.location.href='${basePath}/channel/channelmsg'",2000);
					}else{
						layer.msg("删除通道信息失败！", {time:3000, icon:6, shift:2}, function(){});
					}
				}
			});
			layer.close(cindex);
	...
	function changePageNum(pageNum){
		window.location.href = "${basePath}/channel/channelmsg?pageNum="+pageNum;
	}
</script>
</body>
</html>
