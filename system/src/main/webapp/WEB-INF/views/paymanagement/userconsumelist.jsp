    ...
    *********************************************************************************************************************************************************** -->
	...
	<section id="main-content">
		<section class="wrapper">
			<h3><i class="fa fa-angle-right"></i>财务管理/消费记录</h3>
			<div class="row mt">
				<div class="col-lg-12 col-xs-12">
					<h4><i class="fa fa-angle-right"></i>消费记录信息</h4>
					...
					<div class="panel-body">
						<div class="row">
							...
							<div class="col-lg-5 col-md-5 col-sm-5 mb">
								<button type="button" class="btn btn-warning" onclick="checkconsumelist(1);"><i class="glyphicon glyphicon-search"></i> 查询</button>
							</div>
						</div>
					</div>
				...
				<div class="table-responsive">
					<table class="table  table-bordered">
						...
						<tbody class="consumelistbody">
						<c:forEach items="${consumeList.data}" var="item" varStatus="status">
							<tr>
						...
						<tfoot class="consumelistfoot">
						<tr >
							<td colspan="10" align="center">
								<ul class="pagination" id="consumepagelist">
									<input type="hidden" value="${consumeList.totalPage}" id="totalpage">
									<input type="hidden" value="${consumeList.pageNum}" id="pagenum">
									<input type="hidden" value="${consumeList.pageSize}" id="pagesize">
									<input type="hidden" value="${consumeList.countNum}" id="countnum">
								</ul>
								<ul id="consumelistcount">
								</ul>
								</ul>
							</td>
						</tr>
...
<script>
	...
	$("#consumelistcount").append("<li><span>共"+pagesum+"页"+countnum+"条记录</span></li>")
	...
	function checkconsumelist(pageNum) {
		...
		$.ajax({
			type:"POST",
			url:"${basePath}/pay/checkuserconsumelist",
			data:{"begintime":begintime,"endtime":endtime,"pageNum":pageNum},
			success:function (result) {
				//清空表格数据
				$(".consumelistbody").empty();
				$(".consumelistfoot").empty();
				//总页码
				...
				for (var i = 0; i < dataStr.length ; i++) {
					...
					$(".consumelistbody").append("<tr class='record_tr'>" +
							"<td>"+(i+1)+"</td>" +
							"<td><input type='checkbox' ></td>" +
							"<td>"+dataStr[i].account+"</td>" +
							"<td>"+dataStr[i].beforeBalance+"</td>" +
							"<td>"+dataStr[i].balance+"</td>" +
							"<td>"+dataStr[i].consumeMoney+"</td>" +
							"<td>"+opTime+"</td>" +
							"</tr>");
				}
				//使用插件分页
				$(".consumelistfoot").append("<tr><td colspan='12' align='center'><ul class='pagination' id='consumemsg'></ul><ul id='consumecount'></ul></td></tr>");
				$.jqPaginator('#consumemsg', {
					...
					onPageChange: function (num, type) {
						if (type == "change"){
							checkconsumelist(num);
						}
					}
				});
				$("#consumecount").append("<span>共"+totalPage+"页"+countNum+"条记录</span>")
			}
		});
	}
	//翻页
	function changePageNum(pageNum){
		window.location.href = "${basePath}/pay/cpconsumelist?pageNum="+pageNum;
	}
